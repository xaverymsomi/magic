...
<div id="page-content">
    ...
    foreach ($this->buttons as $button) {
        if ($perm->verifyPermission(strtolower($button['permission']))) { //check permission
            ...
            echo trans($button['title']) . '</button>';
        }
    }
    echo '</div>';
    ...
    echo '<div class="panel-heading" style="color: #fff; background: rgba(0,0,0,0.4)"><h4 class="panel-title">';
    ...
    if ($this->resultData['recordsFiltered'] > 0) {
        ...
        echo $view->displayTBody($this->allRecords, $this->class, $this->table, $this->hidden, $actions, LBL_BIG, $this->labels);
        echo '</table>';
        echo '</div>';
        // Add mabrex pager
        ...
        // Add mabrex filter
        echo '<mabrex-filter mx-selected="' . $this->postData['length'] . '" mx-location="\'' .
            $this->postData['location'] . '\'" mx-title="\'Vehicle Passed\'" mx-current-link="\'' .
            $this->postData['current'] . '\'" mx-page-size="\'' . $this->postData['length'] . '\'" mx-search-term="\'' .
            $this->postData['search'] . '\'" mx-total-records="' . $this->resultData['recordsTotal'] . '" mx-table-columns="' .
            $this->resultData['columns'] . '" mx-sort-column="\'' . $this->postData['order_column'] . '\'" mx-sort-order="\'' .
            $this->postData['order_dir'] . '\'" mx-column-label="\'' . $this->resultData['column_label'] . '\'"></mabrex-filter>';
